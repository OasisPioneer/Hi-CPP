...
- [x] **历史简介**
    - [x] 发展历史：C with Classes -> C++98 -> C++11 (现代分水岭) -> C++20/23。
    - [x] 语言特点：零开销抽象、RAII、高性能、多范式。
    - [x] 应用领域：游戏引擎 (Unreal)、嵌入式、高频交易、AI 基础设施。
    - [x] **🎯 练习**：查阅资料，列出 3 个你常用的软件中哪些是用 C++ 开发的。
- [x] **环境搭建**
    - [x] 编译器：GCC (Linux), MSVC (Windows), Clang (MacOS/LLVM)。
    - [x] 构建工具：**CMake** (行业标准，必须掌握编写 CMakeLists.txt)。
    - [x] 开发工具：Visual Studio (IDE), VS Code (轻量级), CLion (JetBrains)。
    - [x] 包管理器：Vcpkg 或 Conan (现代 C++ 依赖管理)。
    - [x] **🎯 练习**：并在本地安装 GCC 和 CMake，编写一个 `CMakeLists.txt` 来编译 "Hello World"。
- [x] **调试器入门**
    - [x] 调试器使用：GDB (命令行) 或 IDE 内置调试器。
    - [x] 核心技能：设置断点 (Breakpoint)、单步执行 (Step Over/Into)、查看变量。
    - [x] 堆栈跟踪：理解 Call Stack，分析 Crash 时的核心转储 (Core Dump)。
    - [x] **🎯 练习**：编写一个故意会 Crash 的程序（如访问空指针），使用调试器定位崩溃行号。

### 第一阶段：基础语法 (Basic Syntax)

- [x] **基础语法**
    - [x] 数据类型：整数 (int, long long)、浮点 (double)、布尔 (bool)、字符 (char)。
    - [x] 变量常量：变量初始化 (C++11 `{}` 列表初始化)、`const`、`constexpr` (编译期常量)。
    - [x] 运算符：算术、逻辑、位运算、`sizeof`。
    - [x] 流程控制：`if` (带初始化 C++17)、`switch` (fallthrough 特性)、`for` (范围 for 循环)、`while`。
    - [x] **🎯 练习**：编写一个“猜数字游戏”，使用 `while` 循环和 `if` 判断，并用 `const` 定义最大尝试次数。
- [x] **函数与作用域**
    - [x] 函数定义与调用：分离声明 (.h) 与实现 (.cpp)。
    - [x] **参数传递 (核心)**：值传递 vs **引用传递 (`&`)** vs 指针传递。
    - [x] 命名空间：`namespace` 的定义、`using` 声明、避免在头文件使用 `using namespace std`。
    - [x] 函数重载：同名不同参的实现原理 (Name Mangling)。
    - [x] **🎯 练习**：编写一个 `swap` 函数，使用“引用传递”交换两个整数的值；再重载它以支持浮点数。
- [x] **数组与字符串**
    - [x] 数组：C 风格数组的局限性 vs `std::array` (C++11 固定大小数组)。
    - [x] 字符串：`std::string` 的增删改查、拼接、子串查找。
    - [x] 字符串视图：`std::string_view` (C++17 无拷贝字符串切片)。
    - [x] **🎯 练习**：输入一行英文句子，使用 `std::string` 统计其中单词的个数，并反转整个句子输出。

### 第二阶段：面向对象 (OOP)

- [x] **类与对象**
    - [x] 构造与析构：默认构造、初始化列表 (效率更高)、析构函数 (资源释放)。
    - [x] **三/五法则**：拷贝构造、拷贝赋值、移动构造、移动赋值。
    - [x] 静态成员 (`static`)：类共享数据，非特定对象所有。
    - [x] **🎯 练习**：设计一个 `Student` 类，包含姓名和成绩，使用初始化列表构造，并统计当前内存中有多少个 `Student` 对象（使用 static）。
- [x] **封装与访问控制**
    - [x] 访问权限：`public` (接口), `private` (数据), `protected` (子类可见)。
    - [x] `const` 成员函数：保证函数不修改对象状态。
    - [x] **🎯 练习**：创建一个 `BankAccount` 类，余额为 private，提供 `deposit` 和 `withdraw` 方法，确保余额不能为负数。
- [x] **继承与多态 (重难点)**
    - [x] 继承关系：继承的权限 (`public` 继承最常用)。
    - [x] **虚函数 (`virtual`)**：动态多态、虚函数表 (vtable) 原理。
    - [x] 纯虚函数与抽象类：定义接口 (Interface)。
    - [x] `override` 与 `final` (C++11)：显式标记重写与禁止继承。
    - [x] 虚析构函数：防止删除基类指针时子类泄漏。
    - [x] **🎯 练习**：实现 `Shape` 抽象类（含纯虚函数 `area()`），派生出 `Circle` 和 `Rectangle`，用 `Shape*` 指针数组循环计算总面积。
